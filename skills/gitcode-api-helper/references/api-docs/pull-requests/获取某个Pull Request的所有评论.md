...
| direction | string | 否 | 可选。升序/降序(asc/desc) |
| comment_type | string | 否 | 可选。筛选评论类型。代码行评论/pr普通评论:diff_comment/pr_comment |
...
conn.request("GET", "/api/v5/repos/:owner/:repo/pulls/:number/comments", payload, headers)
